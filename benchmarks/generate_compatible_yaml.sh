...
echo "Generated small_117kb.yaml ($(stat -f%z small_117kb.yaml 2>/dev/null || stat -c%s small_117kb.yaml) bytes)"

# Medium file (~500KB)
cp small_117kb.yaml medium_1mb.yaml

# Add services data
cat >> medium_1mb.yaml << 'EOF'
...
echo "Generated medium_1mb.yaml ($(stat -f%z medium_1mb.yaml 2>/dev/null || stat -c%s medium_1mb.yaml) bytes)"

# Large file (~3MB)
cp medium_1mb.yaml large_6_5mb.yaml

# Add events data
cat >> large_6_5mb.yaml << 'EOF'
...
echo "Generated large_6_5mb.yaml ($(stat -f%z large_6_5mb.yaml 2>/dev/null || stat -c%s large_6_5mb.yaml) bytes)"

# Very large file (~10MB)
cp large_6_5mb.yaml very_large_13mb.yaml

# Add transaction data
cat >> very_large_13mb.yaml << 'EOF'
...
echo "Generated very_large_13mb.yaml ($(stat -f%z very_large_13mb.yaml 2>/dev/null || stat -c%s very_large_13mb.yaml) bytes)"
